...
def regression_model(model, df1, df2, name):
  x = df1[['month','year','vinoblanco','vinotinto']]
  y = df1[['revenue']]
  model.fit(x,y)
  x_test = df2[['month','year','vinoblanco','vinotinto']]
  y_test = df2[['revenue']]
...
data = pd.read_csv("C:\\Users\\Ana Bossler\\Desktop\\testing.csv")
df = pd.DataFrame(data)
x = df[['month','year','vinoblanco','vinotinto']]
y = df[['revenue']]
sns.factorplot(x="month", y="revenue", hue="year", data=df)
plt.show()
data2 = pd.read_csv("C:\\Users\\Ana Bossler\\Desktop\\testingval.csv")
